	...
	<ul class="pagination justify-content-end">
		<c:if test="${!pagedListHolder.firstPage}">
			<li class="page-item"><a class="page-link" aria-label="Previous"
				href="<%=StringUtils.replace(pagedLink, "~", String.valueOf(pagedListHolder.getPage() - 1))%>"><span aria-hidden="true">&laquo;</span></a></li>
		</c:if>
		...
		<c:if test="${!pagedListHolder.lastPage}">
			<li class="page-item"><a class="page-link" aria-label="Next"
				href="<%=StringUtils.replace(pagedLink, "~", String.valueOf(pagedListHolder.getPage() + 1))%>"><span aria-hidden="true">&raquo;</span></a></li>
		</c:if>
	</ul>
</c:if>